...
Знакомство с курсом, языком Julia.

# Задание

- Настроить рабочее пространство лабораторной работы.

- Создание проекта DrWatson для лабораторных работ.

- Написать модель экспоненциального роста

# Выполнение лабораторной работы

## Настройка git

Вначале произвел настройку git ([рис. @fig-001]):
...
После сгенерировал и добавил SSH и GPG ключи ([рис. @fig-002]):

SSH:
```
ssh-keygen -t ed25519 -f ~/.ssh/git -C "1032221668@rudn.ru"
ssh-add ~/.ssh/git
```
...
![Настройка ключей SSH и GPG](image/2.png){#fig-002 width=70%}

После настроил git для работы с GPG:
...
## Настройка рабочего пространства лабораторной работы

Вначале установил `quarto`, `Node.js` и `pnpm` для своей системы.
После поставил `git-flow`, `commitizen`, `cz-customizable` 
и `standard-version`.

Данные шаги, в целом, тривиальны. Подробное описания опущено.

Далее создал рабочий каталог, клонировал созданный из шаблона репозиторий и 
настроил его под курс "Математическое моделирование":

```
mkdir -p ~/work/study/2026-1/2026-1==study--mathmod
cd ~/work/study/2026-1/2026-1==study--mathmod

git clone git@github.com:ArsKF/2026-1--study--mathmod.git
echo mathmod > COURSE
make prepare
```

Затем отправил файлы на сервер:
...
После произвел первый "инициализионный" релиз:
...
![Первый релиз](image/3.png){#fig-003 width=70%}

## Создание проекта DrWatson для лабораторных работ

Создал файл `setup_project.jl` в каталоге `labs/lab01`:
...
После запустил его и перешел в каталог:
...
![Создание проекта DrWatson](image/4.png){#fig-004 width=70%}

Затем установил необходимые пакеты и проверил, что все правильно настроилось.

Для этого создал скрипт `add_packages.jl`:
...
Запустил их командами соответственно:
...
![Установка зависимостей](image/5.png){#fig-005 width=70%}

## Описание модели

В данной работе моделируется "Экспоненциальный рост".
Для ее реализации я создал следующий скрипт `scripts/01_exponential_growth.jl`:
...
![Результат выполнения первого скрипта 1](image/6.png){#fig-006 width=70%}

![Результат выполнения первого скрипта 2](image/exponential_growth_α=0.3.png){#fig-007 width=70%}

После изменил его, для добавления описания:
...
После я создал скрипт для генерации производных форматов `scripts/tangle.jl`:
...
Затем выполнил его с первым скриптом:

```
julia --project=. scripts/tangle.jl scripts/01_exponential_growth.jl
```

![Результат выполнения первого скрипта через tangle](image/8.png){#fig-008 width=70%}

Также, получил Jupyter-ноутбук `notebooks/01_exponential_growth/01_exponential_growth.ipynb`.

![Jupyter-ноутбук первого скрипта через tangle](image/9.png){#fig-009 width=70%}

После добавил поддержку кода julia в отчет добавив в `_quatro.yml` следующее:
...
Создал обновленный скрипт `scripts/02_exponential_growth.jl`:
...
И также, получил Jupyter-ноутбук `notebooks/02_exponential_growth/02_exponential_growth.ipynb`.

![Jupyter-ноутбук второго скрипта через tangle](image/10.png){#fig-010 width=70%}
...
Я познакомился с курсом и языком Julia.
